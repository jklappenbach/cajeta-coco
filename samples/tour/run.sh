...
# `cover` measured. Neither needs a cajeta-coco checkout — the plugin resolves
# from Olla like any other dependency, which is the point: this tour is what a
# CONSUMER of coco does, not what coco's developers do.
#
# Override the compiler with CAJETA=/path/to/cajeta (defaults to `cajeta` on
# PATH, matching samples/tour in cajeta-unit and cajeta-logging).
set -uo pipefail
...
if ! command -v "$CAJETA" >/dev/null 2>&1 && [[ ! -x "$CAJETA" ]]; then
    echo "error: cajeta compiler not found ($CAJETA)" >&2
    echo "       install the toolchain, or override with CAJETA=/path/to/cajeta" >&2
    exit 1
fi
