...
Every finding cajeta-coco can report, one class each — and what each one looks
like in the IDE.

This is a **consumer's** project: it declares `dev.cajeta.coverage` as a plugin
and resolves it from Olla like any other dependency. Nothing here builds coco
from this checkout, which is deliberate — the tour is what someone adopting
coco does, and it would be a poor demonstration if it only worked from inside
the repo.
...
```bash
./run.sh               # cajeta cover, then walk the artifacts
```
...
├── run.sh                            ← cover + a walk through the findings
└── src/tour/coco/
...
- [Guide: 23 — Code coverage](https://github.com/jklappenbach/cajeta/blob/main/docs/guide/23-code-coverage.md) — the full
  configuration reference: every setting, where results land, how to invoke a
  pass. (In the **cajeta** repo, with the IntelliJ plugin this tour's IDE
  section describes.)
- [BuildTool.md § Code coverage](https://github.com/jklappenbach/cajeta/blob/main/docs/specification/buildtool/BuildTool.md)
  — the plugin in the context of the action catalog.
- [`cajeta-coco/README.md`](../../README.md) — what the engine does and why.
